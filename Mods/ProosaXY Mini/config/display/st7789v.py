...
# Patched for modern Klipper (2025/2026 compatibility)
# Fixes:
# - Removed use_cs_pin (deprecated)
# - Fixed float/int issues
# - Fixed PIL resize integer requirements
# - Fixed numpy fromstring -> frombuffer
# - Added strict pin validation
# - Removed unsafe ConfigWrapper hacks
...
ST7789V_CMD_IDMOFF = 0x37
ST7789V_CMD_IDMON = 0x38
...
class DisplayBase:
    def __init__(self, send):
        ...
        self.remote_vram = Image.new('RGB', (ST7789V_COLS, ST7789V_ROWS), (255,255,255))
        self.vram = Image.new('RGB', (ST7789V_COLS, ST7789V_ROWS), (0,0,0))

        self.invalidated_bbox = [0, 0, ST7789V_COLS, ST7789V_ROWS]

        x1, y1, x2, y2 = self.render_logo()

        self.ui_width_chars = (ST7789V_COLS - 2*ST7789V_FONT_WIDTH)//ST7789V_FONT_WIDTH
        self.ui_height_chars = (ST7789V_ROWS - y2 - 2*ST7789V_FONT_HEIGHT)//ST7789V_FONT_HEIGHT

        ui_border_x = (ST7789V_COLS - self.ui_width_chars*ST7789V_FONT_WIDTH)/2

        self.ui_top_left = (
            int(ui_border_x),
            int(ST7789V_ROWS - (self.ui_height_chars+1)*ST7789V_FONT_HEIGHT)
        )

        self.ui_bottom_right = (
            int(self.ui_top_left[0] + self.ui_width_chars*ST7789V_FONT_WIDTH),
            int(self.ui_top_left[1] + self.ui_height_chars*ST7789V_FONT_HEIGHT)
        )
        ...
        self.font = [self._rowmajor_bitmap_to_image(b) for b in font8x14.VGA_FONT]
        self.icons = {}
    ...
    def _invalidate(self,xy,size):
        if self.invalidated_bbox is None:
            self.invalidated_bbox=[xy[0],xy[1],xy[0]+size[0],xy[1]+size[1]]
        else:
    ...
    def render_logo(self):
        logo_width = ST7789V_COLS
        if self.logo is None:
            self.logo = logo.make_logo(logo_width)
        x_pos = (ST7789V_COLS - logo_width)//2
        y_pos = 0
        self.vram.paste(self.logo,(int(x_pos),int(y_pos)),self.logo)
        self._invalidate((int(x_pos),int(y_pos)),self.logo.size)
...
class ST7789V(DisplayBase):
    def __init__(self, config):
        ...
        def req(name):
            ...
            if v is None or str(v).strip()=="":
                raise config.error(f"Missing pin: {name}")
            return str(v).strip()
        ...
        self.rs_pin = bus.MCU_bus_digital_out(self.spi.get_mcu(),rs,self.spi.get_command_queue())
        self.mcu_reset = bus.MCU_bus_digital_out(self.spi.get_mcu(),rst,self.spi.get_command_queue())
    ...
    def set_rs(self,v):
        if v is None: return
        if self.rs_pin_state!=v:
            self.rs_pin_state=v
            self.rs_pin.update_digital_out(v,reqclock=BACKGROUND_PRIORITY_CLOCK)
    ...
    def write_data(self,data):
        ...
        while data:
            part=data[:maxlen]
            self.spi.spi_send(part,reqclock=BACKGROUND_PRIORITY_CLOCK)
            data=data[maxlen:]
    ...
    def init(self):
        ...
        # RESET PULSE
        self.mcu_reset.update_digital_out(1)
        self.mcu_reset.update_digital_out(0)
        self.mcu_reset.update_digital_out(1)
